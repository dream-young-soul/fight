...
/////////////////////////////////////////////////////////////////////////////
// CMyShellDlg dialog
int CMyShellDlg::m_nHotkeyValue [10] ;
...
void CMyShellDlg::OnIconDrop( LPARAM lParam )
{
    ...
    case DLG_STONEGOODS0:
        ...
        else if (m_PickUpDlgID == DLG_DEPOT)
        {
            ...
            {
                if ( !CMyShellDlg::IsCanCheckOutBagItem(pOutItem->GetTypeID()) )
                {
                    g_objGameMsg.AddMsg ((char*)g_objGameDataSet.GetStr (10679)) ;
                    m_DlgMain.m_DlgDepot.m_DepotGriLay.ReSumePickUp () ;
                    g_bPickUp = false;
                    return ;
                }
...
void CMyShellDlg::MoveDlg()
{
    CPoint MousePnt ;
    CRect rect ;
    m_DlgTitleBar.GetWindowRect( rect ) ;
    ::GetCursorPos( &MousePnt ) ;
    if ( !rect.PtInRect( MousePnt ) )
    {
        m_bDlgMove = false ;
    ...
    MousePnt = m_DlgTitleBar.GetOffset () ;
    CMyShellDlg::GetParent()->MoveWindow ( m_InitDlgRect.left + MousePnt.x,
                                           m_InitDlgRect.top + MousePnt.y,
                                           m_InitDlgRect.Width (),
...
int CMyShellDlg::ShowItemInfo(CItem* pItem, int x, int y, int& nMaxLen, BOOL bCanShow)
{
    if(pItem != NULL)
    {
        ...
        int nSingle = 0;
        int nLineNum = 0;
        for ( int i = 0; i < strlen ( pItem->GetDesc () ); i++ )
        {
...
BOOL CMyShellDlg::IsCanCheckOutBagItem(OBJID idType)
{
    ...
    if((nSubType / 10000) * 10000 == ITEMTYPE_GHOSTGEM)
    {
        ...
        if (!bCanBuy)
        {
            OBJID idPackageFull = 10679;		//100200=宝石背包已经满了，不能取出
    ...
    else
    {
        ...
        if (!bCanBuy)
        {
            OBJID idPackageFull = 10106 ;		//=错误：物品栏已满。
            g_objGameMsg.AddMsg(g_objGameDataSet.GetStr(idPackageFull));
        }
    }
    return bCanBuy;
