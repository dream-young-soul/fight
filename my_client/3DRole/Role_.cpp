...
BOOL CRole::ProcessAction(void)
{
    ...
    else if (m_Info.iActFrameIndex >= m_Info.iActFrameAmount)
    {
        // map 3d effect of action
        int nTerrain = g_objGameMap.GetTerrain(m_Info.posMap);
        const ActionMapEffectInfo* pInfo = g_obj3DRoleData.GetActionMapEffect(m_Info.usLook, m_Info.iActType, nTerrain);
...
void CRole::Show()
{
    ...
    }
    {
        ...
        if (m_Info.bAdjustDir && (_ACTION_RUNL == m_Info.iActType || _ACTION_RUNR == m_Info.iActType))
        {
            ...
            nRotate = (int)((180.0f * dAtan) / 3.1415926 + 90);
        }
        ...
        {
            DWORD dwTimeGoesBy = ::TimeGet() - m_tmDisappear;
            alpha = (float)(1.0f - 0.2f * dwTimeGoesBy / 1000.0f);
            if (alpha < 0.0)
            {
                alpha = 0.0;
            }
            alphaShadow =(float)( 0.8f - 0.15f * dwTimeGoesBy / 1000.0f);
            if (alphaShadow < 0.0)
            {
                alphaShadow = 0.0;
            }
        ...
        {
            m_pIRoleView->SetRGBA((float)(1.0 * ucA / (255 * alpha * 2)), (float)(1.0 * ucR / 255), (float)(1.0 * ucG / 255),
				(float)(1.0 * ucB / 255));
        }
        else
        {
...
int	CRole::GetJumpHeight()
{
    ...
    nHightestAltitude =(int)(nHightestAltitude + sqrt(float((posBeginWorldPos.x - posEndWorldPos.x) * (posBeginWorldPos.x - posEndWorldPos.x) +
    ...
    if (beUp)
    {
        ...
        double dbDisNow = sqrt(double((posBeginWorldPos.x - posCurrentWorld.x) * (posBeginWorldPos.x - posCurrentWorld.x) +
                                      (posBeginWorldPos.y - posCurrentWorld.y) * (posBeginWorldPos.y - posCurrentWorld.y)));
        double dbDisEnd = sqrt(double((posBeginWorldPos.x - posHighestWorldPos.x) * (posBeginWorldPos.x - posHighestWorldPos.x) +
        ...
        nHeight = (int)(nBeginAltitude + nHeightDef * sin(dbAngle));
    }
    else
    {
        int nHeightDef =  nHightestAltitude - nEndAltitude;
        double dbDisNow = sqrt(double((posEndWorldPos.x - posCurrentWorld.x) * (posEndWorldPos.x - posCurrentWorld.x) +
                                      (posEndWorldPos.y - posCurrentWorld.y) * (posEndWorldPos.y - posCurrentWorld.y)));
        double dbDisEnd = sqrt(double((posEndWorldPos.x - posHighestWorldPos.x) * (posEndWorldPos.x - posHighestWorldPos.x) +
        ...
        double pi = 3.1415926535;
        double dbAngle = pi * dbDisNow / dbDisEnd / 2;
        nHeight =  (int)(nEndAltitude + nHeightDef * sin(dbAngle));
    }
    return nHeight;
}
