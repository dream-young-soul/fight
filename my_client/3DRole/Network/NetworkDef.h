...
const unsigned short _MSG_TASK_LIST		= _MSG_GENERAL + 1040; //任务列表

const unsigned short _MSG_ANNOUNCE_LIST = _MSG_GENERAL + 1041;
const unsigned short _MSG_ANNOUNCE_INFO = _MSG_GENERAL + 1042;

const unsigned short _MSG_AUCTION		= _MSG_GENERAL + 1043; //拍卖

const unsigned short _MSG_ITEMATTRIB    = _MSG_GENERAL + 1045; //用来更新幻兽的经验值和HP值
...
const unsigned short _TXTATR_MY			= _TXTATR_NORMAL + 15; // 有我有关
...
const unsigned short _TXT_SYNWAR_FIRST	= _TXTATR_NORMAL + 108;	// 帮派战绩第一个
const unsigned short _TXT_SYNWAR_NEXT	= _TXTATR_NORMAL + 109;	// 帮派战绩后继

const unsigned short _TXTATR_LEAVEWORD		= _TXTATR_NORMAL + 110; //留言系统
const unsigned short _TXTATR_SYNANNOUNCEG	= _TXTATR_NORMAL + 111; //帮派公告 --游戏中同步
const unsigned short _TXTATR_SYNANNOUNCEL	= _TXTATR_NORMAL + 114; //帮派公告 --登陆时同步

const unsigned short _TXTATR_MESSAGEBOX		= _TXTATR_NORMAL + 112;	// 对话框
const unsigned short _TXTATR_REJECT			= _TXTATR_NORMAL + 113;	// 驳回

const unsigned short _TXTATR_TRACE			= _TXTATR_NORMAL + 115;	// 追踪 服务器10秒1次 客户端15秒timeout
