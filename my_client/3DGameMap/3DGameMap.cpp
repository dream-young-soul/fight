    ...
    for(i = 0; i < infoSize.iHeight; i++)
    {
        for(j = 0; j < infoSize.iWidth; j++)
        {
            ...
            if (pLayer)
            {
                ...
                pLayerInfo->usTerrain = (unsigned short)nTer;
                pLayerInfo->sAltitude = (short)nAltitude;
            }
        }
    }
...
void	CGameMap::BeginColorEffect(int nEffectType)
{
    ...
    ColorEffectInfo.bRed = (BYTE)((ColorEffectInfo.dwOldColor & 0x00ffffff) >> 16);
    ColorEffectInfo.bGreen = (BYTE)((ColorEffectInfo.dwOldColor & 0x0000ffff) >> 8);
    ColorEffectInfo.bBlue =(BYTE)( ColorEffectInfo.dwOldColor & 0x000000ff);
...
{
    for(int i = 0; i < (int)m_setRegionEffect.size(); i++)
    {
        RegionEffectInfo* pInfo = m_setRegionEffect[i];
...
void CGameMap::ProcessRegionEffect(CMyPos posCell)
{
    ...
    else if (nIndex == -1 && m_nCurRegionEffectIndex != -1)
    {
        ...
        if ((int)(::TimeGet() - m_dwTimeRegionEffect) > pInfo->nDelayTime)
        {
            m_objSnow.Clear();
            m_nCurRegionEffectIndex = -1;
            m_dwTimeRegionEffect = 0;
    ...
    else if (nIndex != -1 &&  nIndex  != m_nCurRegionEffectIndex)
    {
        ...
        else if ((int)(::TimeGet() - m_dwTimeRegionEffect) >= pInfo->nDelayTime)
        {
